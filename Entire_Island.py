...
#fig.suptitle('ERA5 wind and solar daily capacity factors 2018 entire island', fontsize=16)
...
Solar_CF = np.nanmean(av_output_solar_month, axis = (0,1))
Wind_CF = np.nanmean(av_output_wind_month, axis = (0,1))
...
#plt.title('Monthly ERA5 solar and wind capacity factors 2018')
plt.grid(True)
plt.legend()
...
plt.plot(months, Cstab, label='Cstab', marker='o', color = 'orange')

#plt.ylim(0,1)
plt.errorbar(months,Cstab, yerr, color = 'orange')
plt.xlabel('Months')
plt.ylabel('Stability Factor')
#plt.title('Monthly ERA5 stabilty factors 2018')
...
#title = 'ERA5 wind capacity factor 2018'
cmap = 'Blues'
...
#title = 'ERA5 solar capacity factor 2018'
cmap = 'Reds'
cbar_label = 'Capacity factor'
projection = ccrs.PlateCarree()
...
# strider = np.nanmean(coeff_stab_diurnal, axis = 2)
...
# title = 'ERA5 stability factor 2018'
# cmap = 'Greens'
...
# # Example data (replace this with your NumPy array)
# data_array = map_output_wind

# # Define the output raster file path
# output_path = 'output_raster.tif'

# # Define the spatial information (replace this with your spatial information)
# # Example spatial information: (top-left x, top-left y, pixel size x, pixel size y)
# spatial_info = (0, 0, 1, -1)  # Assuming a simple case where each pixel represents a unit area

# # Create a GeoTransform from the spatial information
# transform = from_origin(spatial_info[0], spatial_info[1], spatial_info[2], spatial_info[3])

# # Create a GeoTIFF file
# with rasterio.open(output_path, 'w', driver='GTiff', height=data_array.shape[1], width=data_array.shape[2],
#                    count=data_array.shape[0], dtype=data_array.dtype, crs='EPSG:4326', transform=transform) as dst:
