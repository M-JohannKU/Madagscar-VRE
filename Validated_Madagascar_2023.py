...
filename = r"C:\Users\Asus\Desktop\Thesis\ERA5\Madagascar_5_years_componants\MAD_2023_5year_componant.nc"

#designation of datasource
ds = xr.open_dataset(filename, decode_times=False)

#2023 data weird, need reduce to remove expver
...
# Loop over days within the current month
    ...
    for m in range(len(months)):
        ...
        #coeff_stab_diurnal[y,z] = 1 - ((np.nanmean(variance_combo_diurnal[m,:]))/(np.nanmean(variance_solar_diurnal[m,:])))
...
# Define the colors for the lines (example colors)
...
fig.suptitle('Daily ERA5 wind and solar capacity factors 2023', fontsize=16)

# Adjust spacing between subplots
plt.tight_layout()

#saving figure 
plt.savefig('Madagascar', dpi=300, bbox_inches='tight')
...
plt.plot(months, Solar_CF, label='Solar', marker='o', linestyle='-',color='orange')
plt.plot(months, Wind_CF, label='Wind', marker='o', linestyle='-',color='blue')

plt.xlabel('Months')
plt.ylabel('Capacity Factor')
plt.title('Monthly ERA5 solar and wind capacity factors 2023')
plt.grid(True)
plt.legend()
...
plt.title('Monthly ERA5 stabilty factors 2023')
...
np.save('CF_wind_2023.npy', Wind_CF)
np.save('CSTAB_2023.npy', Cstab)
np.save('Wind_CF_2023.npy',Wind_CF)
np.save('Solar_CF_2023.npy',Solar_CF)
np.save('yerr_2023',yerr)