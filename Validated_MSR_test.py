...
av_coeff_stab_diurnal_all = np.zeros(shape = (np.size(dummy_latlonmon,0), np.size(dummy_latlonmon,1), np.size(dummy_latlonmon,2)))

max_output_wind_all =np.zeros(shape = (np.size(dummy_latlonmon,0), np.size(dummy_latlonmon,1), np.size(dummy_latlonmon,2)))
max_coeff_stab_diurnal_all = np.zeros(shape = (np.size(dummy_latlonmon,0), np.size(dummy_latlonmon,1), np.size(dummy_latlonmon,2)))
...
##Imports numpy  array of MSR hourly capcity factors
MSR_wind_CF = np.load(r"C:\Users\Asus\Desktop\Thesis\MSR_wind_99.npy",allow_pickle=True)
MSR_solar_CF = np.load(r"C:\Users\Asus\Desktop\Thesis\MSR_sun_117.npy",allow_pickle=True)
 
#### Appends input arrays to use MSR CFs rather than ERA5 CFs
map_output_solar[:, :, :] = MSR_solar_CF[:, np.newaxis, np.newaxis]

map_output_wind[:, :, :] = MSR_wind_CF[:, np.newaxis, np.newaxis]
##Specific coordinates for Taiba Ndaiye, use 6,6 because it works, attempting others to get ctab cooperating
y=31 #use highest CF wind location from MSR lat -17.8656
z=16    #same highest CF from MSR wind long 47.03262
...
# Loop over days within the current month
    ...
    #av_coeff_stab_diurnal_all[y,z] = np.nanmean(coeff_stab_diurnal[y,z,:])
    #max_coeff_stab_diurnal_all[y,z] = np.nanmax(coeff_stab_diurnal[y,z,:])
...
#fig.suptitle('MSR wind and solar daily capacity factors 2018', fontsize=16)
...
Solar_CF = av_output_solar_month[y,z,:]
Wind_CF = av_output_wind_month[y,z,:]

plt.figure(figsize=(5,3))

plt.plot(months, Solar_CF, label='Solar', marker='v', linestyle='-',color='orange')
plt.plot(months, Wind_CF, label='Wind', marker='s', linestyle='-',color='blue')

plt.xlabel('Months')
plt.ylabel('Capacity Factor')
#plt.title('Monthly MSR solar and wind capacity factors 2018')
plt.grid(True)
plt.legend()
...
CSTAB_ERA5_2018 = np.load(r"C:\Users\Asus\Desktop\Thesis\CSTAB_2018.npy",allow_pickle=True)
CSTAB_ERA5_yerr_2018 = np.load(r"C:\Users\Asus\Desktop\Thesis\yerr_2018.npy",allow_pickle=True)
#### Ctab vs months
...
plt.plot(months, Cstab, label='Cstab-MSR', marker='o',color = 'green')
plt.plot(months, CSTAB_ERA5_2018, label='Cstab-ERA5', marker='s', color = 'purple')

#plt.ylim(0,1)
plt.errorbar(months,Cstab, yerr, color = 'green')
plt.errorbar(months,CSTAB_ERA5_2018,CSTAB_ERA5_yerr_2018, color = 'purple')
plt.xlabel('Months')
